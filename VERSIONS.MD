...
- Bot API:
    - The IBaseBot´s `getEvents()`/`Events` has been replaced by `getRemainingEvents()`/`RemainingEvents` to provide
      an ordered list of event that has not been processed yet in event handlers.
