...
Two operational prerequisites sit outside this change and belong to the milestones it tracks, not to its own acceptance. First, `rumble-client#9` needs a maintainer merge before M-011 can close; agents never merge their own pull requests (C-002). Second, M-014's documentation sweep is deliberately gated on M-011 through M-013 and on M-015 landing, because re-reading the guides against pre-fix behavior, or against a runtime story that only covers Docker, would bake in the same inaccuracies it is meant to remove.
