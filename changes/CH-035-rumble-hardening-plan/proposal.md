...
- **M-014 — Documentation accuracy sweep**, gated on M-011 through M-013 and on M-015 landing: re-read every Rumble doc against the system's actual post-fix behavior and correct anything left over from before the fixes.
- **M-015 — Container image runs under rootless Podman.** The Docker-based container path has only ever been exercised on Docker Desktop; rootless Podman is the default container runtime on Fedora and RHEL, and `docker/rumble.sh`'s `--user "$(id -u):$(id -g)"` bind-mount pattern is a known risk under Podman's subuid remapping. Tracked in [`robocode-dev/rumble-client#11`](https://github.com/robocode-dev/rumble-client/issues/11).
