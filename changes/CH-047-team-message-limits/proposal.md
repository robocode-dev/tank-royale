...
Classic teams can send more messages in a turn than Tank Royale currently forwards. The Bot APIs, server, and schema disagree about both count and size, so a client can accept a message that the server silently drops. The first 128-message trial skipped turns under the normal deadline. Trial a revised policy for every Tank Royale bot: at most 64 messages and 256 KiB of compact, UTF-8-encoded `teamMessages` array data per bot per turn, and at most 48 KiB for each encoded message. Bound inbound WebSocket text to 1 MiB before JSON parsing. Reject an invalid intent in full rather than delivering a prefix.

The user supplied this trial plan. It is plan-less in the Tank Royale corpus; accepted messaging criteria will be added only after the trial passes. The trial uses local builds only. The policy becomes publishable only if boundary, malformed-client, CombatTeam, and five-bot 30 TPS stress evidence pass.
