...
> 1. Read `.agents/principles-catalog/active.md`; keep if it contains the generated-by marker (`<!-- generated by /dot-scout`). If found, continue.
...
1. **`.agents/principles-catalog/active.md`** — written by `/dot-scout` Phase 6 on every run; present whenever the catalog is vendored. Check for the `<!-- generated by /dot-scout` marker.
...
3. `@group` — read `.agents/principles-catalog/groups/<group>.yaml`; add all IDs from its `principles:` list to the active set; recursively expand any `includes:` entries (detect and abort on cycles).
