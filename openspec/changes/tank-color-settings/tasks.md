...
- [x] Create `gui/src/main/kotlin/dev/robocode/tankroyale/gui/settings/TankColorMode.kt`
...
- [x] Update `gui/src/main/kotlin/dev/robocode/tankroyale/gui/settings/ConfigSettings.kt`
...
- [x] Update `gui/src/main/kotlin/dev/robocode/tankroyale/gui/ui/config/GuiConfigDialog.kt`
...
- [x] Update `gui/src/main/kotlin/dev/robocode/tankroyale/gui/ui/arena/Tank.kt`
...
- [x] Update `gui/src/main/kotlin/dev/robocode/tankroyale/gui/ui/arena/ArenaPanel.kt`
  - Apply the same `resolveColor` pattern for bullet and scan color resolution

## Phase 4 — Tests

- [x] Add unit tests for `TankColorMode` in `gui` module
  - `fromString()` returns correct enum value for each name
  - `fromString()` falls back to `BOT_COLORS` for unknown/null input
- [x] Add unit tests for `Tank` color resolution (mock `ConfigSettings` and `BotState`)
  ...
  - `BOT_COLORS_ONCE`: a second different color from the bot is ignored (cache unchanged)
  - `BOT_COLORS_ONCE`: cache is cleared on `reset()` (new battle); locked color is gone after reset
  - `BOT_COLORS_ONCE`: cached color persists across a simulated round boundary (no reset between rounds)
...
- [x] Verify `PropertiesConsistencyTest` still passes (new `tank-color-mode` key must be covered)
