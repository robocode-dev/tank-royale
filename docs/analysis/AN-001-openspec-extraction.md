...
- ADRs keep their identity as ADR-001…ADR-041, converted to `status: verified` with `accepted-by: Flemming N. Larsen (<date>, pre-Cliewen MADR acceptance)` — the MADR acceptance predates the corpus and is preserved, not re-judged.
