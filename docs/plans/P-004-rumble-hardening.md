...
| M-016 | Container runtime lanes stay current | The primary image uses the LTS-first Java, .NET, Python, and Node.js lanes selected by the runtime policy, and a scheduled refresh proposes a pull request only after all four containerized bot smoke battles and hardened checks pass | todo | CH-038 |

M-012, M-013, M-015, and M-016 are independent of each other and of M-011, and may proceed in parallel. M-014 is gated on M-011 through M-013 and on M-015: re-reading the guides against pre-fix behavior, or against a runtime story that only covers Docker, would bake in the same inaccuracies the sweep exists to remove. M-011 depends only on maintainer merges; agents never merge their own pull requests ([C-002](../constraints/C-002-review-boundary.md)).
