...
| M-015 | Container image runs under rootless Podman | `validate`, `runtimes`, and `sync` complete under rootless Podman on Linux with a writable state directory; every flag difference from Docker is either handled by the launcher scripts or documented for Podman users | todo | [`robocode-dev/rumble-client#11`](https://github.com/robocode-dev/rumble-client/issues/11); only Docker Desktop has been exercised so far |
