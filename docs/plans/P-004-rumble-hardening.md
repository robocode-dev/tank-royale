...
M-012 and M-013 are independent of each other and of M-011, and may proceed in parallel. M-014 is deliberately gated on all three: re-reading the guides against pre-fix behavior would bake in the same inaccuracies the sweep exists to remove. M-011 depends only on maintainer merges; agents never merge their own pull requests ([C-002](../constraints/C-002-review-boundary.md)).
