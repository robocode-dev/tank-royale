...
# ADR-038: Cross-Platform Test Parity and Shared Test Definitions

**Legacy source acceptance:** Flemming N. Larsen (2026-04-14, pre-Cliewen MADR acceptance).

## Context

ADR-003 requires "1:1 semantic equivalence" across all Bot API platforms (Java, C#, Python, TypeScript). ADR-004 makes Java the authoritative reference. However, there is **no mechanism to enforce test parity** — each platform writes its own tests independently, leading to:
...
**Prerequisite:** ADR-037 (Functional Core Extraction) provides a pure, I/O-free validation layer that can be tested without WebSocket or threads.

**References:**
- [ADR-003: Cross-Platform Bot API Strategy](./ADR-003-cross-platform-bot-api-strategy.md)
- [ADR-004: Java as Reference Implementation](./ADR-004-java-reference-implementation.md)
- [ADR-037: Functional Core Extraction](./ADR-037-functional-core-bot-api-testability.md)
...
- ✅ Schema-driven project (ADR-006) already uses JSON schemas
...
2. **Code generation from a spec** — Rejected per ADR-003. Generated test code is hard to debug.
...
- Enforces ADR-003/0004 semantic equivalence mechanically, not by convention
...
- [ADR-004: Java as Reference Implementation](./ADR-004-java-reference-implementation.md)
- [ADR-037: Functional Core Extraction](./ADR-037-functional-core-bot-api-testability.md)
- [ADR-006: Schema-Driven Protocol Contracts](./ADR-006-schema-driven-protocol-contracts.md)
