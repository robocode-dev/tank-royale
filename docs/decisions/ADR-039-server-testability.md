...
# ADR-039: Server Testability — Physics Core Extraction and Test Framework

**Legacy source acceptance:** Flemming N. Larsen (2026-04-14, pre-Cliewen MADR acceptance).

## Context

The game server (`server/`) is the authoritative source for all game physics (ADR-008), turn processing, collision detection, scoring, and event generation. It comprises 86 source files (~6,000+ lines), with the most critical components being:
...
**References:**
- [ADR-008: Server-Authoritative Physics](./ADR-008-server-authoritative-physics.md)
- [ADR-037: Functional Core Extraction (Bot API)](./ADR-037-functional-core-bot-api-testability.md)
- [ADR-012: Turn Timing Semantics](./ADR-012-turn-timing-semantics.md)
...
**Why a separate ADR from ADR-037:** ADR-037 addresses Bot API testability (client-side, cross-platform). Server testability is a distinct concern: single platform (Kotlin), different architecture (game loop vs. intent building), different test infrastructure (Kotest vs. JUnit/NUnit/pytest). The functional-core principle is shared, but the implementation differs.
...
- [ADR-012: Turn Timing Semantics](./ADR-012-turn-timing-semantics.md)
- [ADR-037: Functional Core Extraction (Bot API)](./ADR-037-functional-core-bot-api-testability.md)
- [ADR-038: Cross-Platform Test Parity](./ADR-038-shared-cross-platform-test-definitions.md)
