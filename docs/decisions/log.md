...
| 2026-07-19 | Drop the never-built
pmPublishDryRun` task from CAP-013 (retire TNP-001) instead of building it |
pmPack` already produces the `.tgz` and shows what would be published without uploading, so a dry-run task is redundant | CH-002 |
| 2026-07-19 | TypeScript npm publishing authenticates via the
pmjs-api-key` Gradle property, not an `NPM_TOKEN` env var (retire TNP-002/003, remint TNP-005/006) | Reconciles the corpus to the shipped code and matches the Sonatype/PyPI credential pattern and the `/release` skill | CH-002 |
