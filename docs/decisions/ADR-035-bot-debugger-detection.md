...
# ADR-035: Bot API Debugger Detection

**Legacy source acceptance:** Flemming N. Larsen (2026-04-07, pre-Cliewen MADR acceptance).

## Context

ADR-033 and ADR-034 add server-side debug mode and per-bot breakpoint mode, controlled by a controller (GUI, TUI, etc.). The developer must manually enable breakpoint mode for their bot from the controller UI.
...
**The bot doesn't request anything.** It reports a fact: "a debugger is attached to my process." The controller decides what to do with that information. This preserves the role separation from ADR-007.
...
sequenceDiagram
    ...
    Note over Server: If breakpointModeSupported: auto-enable breakpoint mode for bot X (ADR-034)
```
...
debuggerAttached:
  description: >
    ...
    (ADR-034), the server auto-enables breakpoint mode for this bot.
...
- **Role separation (ADR-007):** The bot plays the game. The bot reports facts; the server acts on them. The bot never requests specific server behavior — it only reports debugger attachment status.
...
- When `breakpointModeSupported` is true (ADR-034), auto-enable breakpoint mode for bots that report `debuggerAttached: true` at game start.
...
**Rejected** — violates role separation (ADR-007). The bot shouldn't control server behavior. The controller decides.
...
- **Composable** — feeds into ADR-034 (breakpoint mode) naturally.
...
- **ADR-034:** Breakpoint Mode — per-bot timeout suspension, server-side `breakpointModeSupported` configuration
- **ADR-033:** Server Debug Mode — pause-after-every-turn
- **ADR-007:** Client Role Separation — bot reports, server acts
