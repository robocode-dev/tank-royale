...
2. **Installation site:** install the override once, in whichever context actually executes `bot.run()` — never merely inside a function both contexts happen to call. `bot.run()` runs only inside the Worker (`startAsWorker()` → the worker bootstrap loop), so that is where worker-mode capture installs. The non-worker/browser path shares `startAsMain()`'s `connect()` call with the worker-spawning main thread, but only the former ever executes `bot.run()`; capture there installs precisely when no Worker was spawned (`mainPort` stays unset), not unconditionally inside `connect()`. A worker-spawning main thread never installs the override, so its own diagnostic `console.error` calls (e.g. worker-error logging) are never captured or rerouted.
3. **Pass-through:** the overridden methods still forward every call to the real underlying `console` method, exactly as Java's `RecordingPrintStream` forwards to the real `System.out`/`System.err` before buffering. A bot author running under plain `node` or in browser devtools keeps seeing their own output locally; capture is additive, not a redirect.
4. **Crossing the Worker boundary:** no new channel. The captured buffers are drained into `stdOut`/`stdErr` fields on the same intent object that already crosses via `postMessage`, at the point `debugGraphics` is already attached (`renderGraphicsToIntent`, called from both `sendIntentToMain` and `sendIntentDirect`). The main thread never touches console capture directly; it only forwards whatever the intent already carries to `WebSocketHandler.sendBotIntent`.
5. **Round-end residual:** mirroring the Java reference's second drain call, output produced by final-tick event handlers after the round's last intent has already gone out is drained into the intent object immediately after the round-ended final dispatch, so it rides on the next round's first intent instead of being lost.
6. **Restoration:** the installed console override is reverted when the bot stops or disconnects. Unlike Java's process-lifetime `System.setOut`, a non-worker/browser bot shares its JS realm with the host page, so leaving `console.log` rerouted after the bot stops would leak into other code sharing that page.

## Consequences

M-017 implements a small capture wrapper around the four `console` methods, installed at the two sites above, forwarding to the real `console` as it captures, and drained at the three points above. Direct writes to `process.stdout`/`process.stderr` that bypass `console` are not captured — a disclosed divergence from the other three APIs' byte-for-byte pipe capture, justified because idiomatic TS bot code does not write to the raw stream and no such stream exists in a browser to capture symmetrically. Multi-argument and non-string `console.log` calls are formatted the way `console` itself would before being appended, so the captured text matches what a bot author sees in a terminal or devtools and the field stays a plain string as the schema requires.
