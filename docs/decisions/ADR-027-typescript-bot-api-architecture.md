...
# ADR-027: TypeScript Bot API for Web Platform Support
...
**Problem:** How should Tank Royale support JavaScript/TypeScript bot development and browser-based competitive environments, while maintaining 1:1 semantic equivalence with the Java reference implementation (ADR-004)?

**Constraints:**

- Must maintain 1:1 semantic equivalence with the Java reference implementation (ADR-003, ADR-004)
- Must support JavaScript/TypeScript as the developer-facing language
- Must enable browser-based bot development (ADR-023)
...
- async/await for blocking methods (see ADR-028)
- Runtime abstraction for Node.js vs browser differences (see ADR-029)
- npm distribution: `@robocode.dev/tank-royale-bot-api`

**Build integration:**

- Gradle orchestrates the TypeScript build via the Node.js Gradle plugin (consistent with monorepo strategy, ADR-001)
...
ADR-003 it has been amended to list TypeScript as the 4th supported language alongside Java, C#, and Python.

---

## References

- [ADR-003: Cross-Platform Bot API Strategy](./ADR-003-cross-platform-bot-api-strategy.md)
- [ADR-004: Java as Authoritative Reference Implementation](./ADR-004-java-reference-implementation.md)
- [ADR-009: WebSocket Communication Protocol](./ADR-009-websocket-communication-protocol.md)
- [ADR-023: Platform Scope and Boundaries](./ADR-023-robocode-tank-royale-platform-scope.md)
- [ADR-028: TypeScript Bot API Threading Model](./ADR-028-typescript-bot-api-threading-model.md)
- [ADR-029: TypeScript Bot API Runtime Targets](./ADR-029-typescript-bot-api-runtime-targets.md)
