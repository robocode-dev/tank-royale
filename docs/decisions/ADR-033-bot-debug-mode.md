...
pauseCause:
  description: >
    ...
    "breakpoint" — the server paused waiting for a bot's intent in breakpoint mode (ADR-034).
  type: string
  enum:
    - pause
    - debug_step
...
Both debug mode and breakpoint mode (ADR-034) reuse `game-paused-event-for-observer`. The `pauseCause` field lets controllers display the correct UI state (e.g. "Paused — debug step" vs. "Paused — waiting for bot X").
...
- **Breakpoints still cause skipped turns without ADR-034** — debug mode alone does not protect a bot that hits a breakpoint mid-turn. The turn timeout still fires, and the bot receives `SkippedTurnEvent`. This is addressed by ADR-034 (Breakpoint Mode), which adds per-bot breakpoint protection on top of debug mode.
...
- **ADR-007:** Client Role Separation — controllers control the server, bots play
- **ADR-012:** Turn Timing Semantics — turn timeout and two-phase timing model
- **ADR-011:** Real-Time Game Loop Architecture — game loop that debug mode interrupts
- **ADR-006:** Schema-Driven Protocol Contracts — schema changes follow this pattern

## References

- [GitHub Issue #204](https://github.com/robocode-dev/tank-royale/issues/204) — Original feature request
- [Battle Lifecycle Flow](/docs/architecture/models/flows/battle-lifecycle.md#manual-pause) — Pause/resume/step sequences
