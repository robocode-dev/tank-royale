...
# ADR-029: TypeScript Bot API Runtime Targets

**Legacy source acceptance:** Flemming N. Larsen (2026-03-24, pre-Cliewen MADR acceptance).

## Context

The TypeScript Bot API (ADR-027) is a pure TypeScript library targeting the web platform. It must interact with runtime-specific APIs for:
...
ADR-023 envisions both server-side bot execution and "browser-based development and competitive environments." Both are first-class goals.
...
- Configuration via `process.env` (matching Java's env var names per ADR-013)
...
- ✅ **Browser support is a primary goal** (ADR-023) — deferring it risks an architecture that's hard to adapt later
...
- Browser support is a primary motivation for the TypeScript Bot API (ADR-023)
...
- No `process.env` configuration breaks compatibility with existing bot launching (ADR-013)
...
- [ADR-013: Bot Configuration via Environment Variables](./ADR-013-bot-configuration-env-vars.md)
- [ADR-023: Platform Scope and Boundaries](./ADR-023-robocode-tank-royale-platform-scope.md)
- [ADR-027: TypeScript Bot API for Web Platform Support](./ADR-027-typescript-bot-api-architecture.md)
