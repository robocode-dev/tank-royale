...
# ADR-034: Breakpoint Mode

**Legacy source acceptance:** Flemming N. Larsen (2026-04-07, pre-Cliewen MADR acceptance).

## Context

ADR-033 introduces **server debug mode** — the server pauses after every turn, letting a developer step through the game turn-by-turn from a controller. This supports post-turn inspection but doesn't help when a bot hits a breakpoint *during* a turn: the bot thread freezes, the turn timeout expires, and the bot receives a `SkippedTurnEvent`.
...
6. **Server receives bot X's intent** — the server processes the turn and continues (or re-pauses if debug mode from ADR-033 is also active).
...
### Composition with Debug Mode (ADR-033)
...
```yaml
# Inside the features object from ADR-033
breakpointMode:
  description: >
...
### 3. `game-paused-event-for-observer` — Reuses `pauseCause` from ADR-033

ADR-033 adds a `pauseCause` field (`"pause"`, `"debug_step"`, `"breakpoint"`) to `game-paused-event-for-observer`. Breakpoint pauses use `pauseCause: "breakpoint"`, letting the controller display "Paused — waiting for *BotName* (breakpoint)" instead of a generic pause message.
...
4. **`GameServer.handleBotIntent()`** — If game is in breakpoint pause and the awaited bot sends its intent:
   - Process the turn.
   - Auto-resume (or stay paused if debug mode from ADR-033 is active).
...
- Auto-enable for bots with debugger attached (ADR-035) is skipped
...
When debug mode (ADR-033) is active, no bot ever gets `SkippedTurnEvent`.
...
- **Composable with debug mode** — ADR-033 (turn-by-turn stepping) + ADR-034 (breakpoint protection) together provide a complete debugging experience.
...
- **ADR-033:** Server Debug Mode — pause-after-every-turn, `features` in server handshake
- **ADR-007:** Client Role Separation — controllers manage per-bot policies
- **ADR-012:** Turn Timing Semantics — turn timeout mechanics
