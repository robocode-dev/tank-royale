...
```kotlin
// Async battle control — pause, step, resume
val owner = Any()
...
BattleRunner.create { embeddedServer() }.use { runner ->
    runner.startBattleAsync(setup, bots).use { handle ->
        handle.onTickEvent.on(owner) { tick ->
            if (tick.turnNumber == 5 && controlled.compareAndSet(false, true)) handle.pause()
        }
        handle.onGamePaused.on(owner) { _ ->
            repeat(3) { handle.nextTurn() }
            handle.resume()
        }
...
// Intent diagnostics — query captured intents per bot per turn
BattleRunner.create { embeddedServer(); enableIntentDiagnostics() }.use { runner ->
    runner.runBattle(setup, bots)
    val store = runner.intentDiagnostics ?: return
    for (botName in store.botNames()) {
        store.getIntentsForBot(botName).forEach { ci ->
            println("${ci.botName} r=${ci.roundNumber} t=${ci.turnNumber} speed=${ci.intent.targetSpeed}")
        }
    }
}
```
...
1. **Module name: `runner`** — Follows the short naming convention of existing modules (`server`, `booter`, `recorder`, `gui`). Not "control" (conflicts with existing Controller role in ADR-007), not "remote" (implies network-only). The API is called "Battle Runner" but the module directory is simply `runner`.
...
8. **Game type presets** — Battle configuration uses the existing game type preset system (see [ADR-025](./ADR-025-game-type-presets-and-rule-configuration.md)): `classic`, `melee`, `1v1`, and `custom`. Selecting a preset provides sensible defaults; individual parameters can be overridden. The simplest call is `runner.runBattle(BattleSetup.classic(), bots)`.

9. **Event delivery via Event\<T\> system** — Reuses the existing event infrastructure from ADR-022, providing consistent patterns across the codebase.
...
|-----------|-----------------|
| Control API | Conflicts with existing "Controller" role (ADR-007) — would create confusion |
...
- Breaks the network-first architecture (ADR-009)
...
- [ADR-005: Independent Deployable Components](./ADR-005-independent-deployable-components.md) — Battle Runner composes these components
- [ADR-007: Client Role Separation](./ADR-007-client-role-separation.md) — Battle Runner acts as Observer + Controller
- [ADR-009: WebSocket Communication Protocol](./ADR-009-websocket-communication-protocol.md) — Underlying communication
- [ADR-022: Event System for GUI Decoupling](./ADR-022-event-system-gui-decoupling.md) — Event delivery pattern reused
- [ADR-023: Platform Scope](./ADR-023-robocode-tank-royale-platform-scope.md) — Tank Royale will not ship tournament tooling itself, but the Battle Runner is the explicit enabler for 3rd-party tournament systems to be built on top (analogous to the `RobocodeEngine` / Control API in classic Robocode)
- [ADR-025: Game Type Presets](./ADR-025-game-type-presets-and-rule-configuration.md) — Preset system used for battle configuration
