...
# ADR-037: Functional Core Extraction for Bot API Testability
...
- [ADR-003: Cross-Platform Bot API Strategy](./ADR-003-cross-platform-bot-api-strategy.md) — requires 1:1 semantic equivalence
- [ADR-004: Java as Reference Implementation](./ADR-004-java-reference-implementation.md) — Java is authoritative
- [ADR-010: Declarative Bot Intent Model](./ADR-010-declarative-bot-intent-model.md) — intent-based architecture
...
- ✅ **Enables shared test definitions** — all platforms can test the same pure logic against the same test data (see ADR-038)
...
3. **Generate code from a single source** — Rejected (per ADR-003). Generated code is hard to debug and customize.
...
- [ADR-010: Declarative Bot Intent Model](./ADR-010-declarative-bot-intent-model.md)
