...
# ADR-026: Identity-Based Bot Matching in Battle Runner

**Legacy source acceptance:** Flemming N. Larsen (2026-03-21, pre-Cliewen MADR acceptance; last updated 2026-04-06).

## Context

The Battle Runner API ([ADR-024](./ADR-024-battle-runner-api.md)) waits for bot processes to connect before starting a battle. The current implementation in `BattleRunner.waitForBots()` uses **count-based** detection:
...
- [ADR-024: Battle Runner API](./ADR-024-battle-runner-api.md) — The Battle Runner architecture this decision extends
- [ADR-016: Session ID for Bot Process Identification](./ADR-016-session-id-bot-process-identification.md) — Related bot identification mechanism (session-level, not directory-level)
