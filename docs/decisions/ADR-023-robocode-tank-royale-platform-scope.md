...
# ADR-023: Robocode Tank Royale Platform Scope and Boundaries
...
3. **Bot APIs** — Language bindings for writing competitive bots:
   - **Current:** Java, Python, .NET, TypeScript (Node.js and browser — see [ADR-027](./ADR-027-typescript-bot-api-architecture.md))
...
- **TypeScript** (stable, Node.js and browser — see [ADR-027](./ADR-027-typescript-bot-api-architecture.md))
...
- **ADR-001** (Monorepo Build Strategy) — Why all core components are in one repository
- **ADR-003** (Cross-Platform Bot API Strategy) — Why we provide Java, Python, and .NET APIs
- **ADR-004** (Java Reference Implementation) — Why Java/Kotlin is authoritative
- **ADR-005** (Independent Deployable Components) — Each component can be replaced or redeployed independently
- **ADR-006** (Schema-Driven Protocol Contracts) — All components communicate via published schemas
...
- [Cross-Platform Bot API Strategy (ADR-003)](./ADR-003-cross-platform-bot-api-strategy.md)
