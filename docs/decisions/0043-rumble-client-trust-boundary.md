...
type: decision
status: inferred
links: [P-003, M-008, CAP-016, CAP-015]
...
The append-only local journal is the durability boundary and issue-ops is the only V1 submission transport. A client credential is limited to Issues access on the result-data repository and never grants repository-content, branch, release, package, Pages, fact, or projection write access. Fork-pull-request submission is introduced only if the result-data contract later supports it.
