...
# ADR-036: Start-Game Debug Options

**Legacy source acceptance:** Flemming N. Larsen (2026-04-08, pre-Cliewen MADR acceptance).

## Context

ADR-033 adds server debug mode (pause-after-every-turn) and ADR-034 adds per-bot breakpoint mode. Both are runtime commands: a controller sends `enable-debug-mode` or `bot-policy-update` while the game is running.
...
debugMode:
  description: >
    If true, the server starts the game in debug mode (ADR-033).
...
After the game starts in debug mode, the battle view shows the debug controls already described in ADR-033 (Step button, etc.).
...
"Start paused" using `pause-game` semantics (TPS=0, game loop stopped) would prevent bots from receiving any ticks. The developer wouldn't see state or events — just a frozen game. Debug mode (ADR-033) is what they actually want: turn 1 executes normally, then the server pauses so they can inspect.
...
- `enable-debug-mode` / `disable-debug-mode` messages (ADR-033) remain valid at runtime — `debugMode` in `start-game` is a convenience shortcut for the startup case, not a replacement.

---

## Related Decisions

- **ADR-033:** Server Debug Mode — defines debug mode behavior and `enable-debug-mode` / `disable-debug-mode`
- **ADR-034:** Breakpoint Mode — per-bot breakpoint protection, `bot-policy-update`
- **ADR-035:** Bot API Debugger Detection — `debuggerAttached` flag, controller auto-enable
- **ADR-007:** Client Role Separation — controllers manage game state, not bots
...
- [ADR-033](./ADR-033-bot-debug-mode.md) — Server Debug Mode
