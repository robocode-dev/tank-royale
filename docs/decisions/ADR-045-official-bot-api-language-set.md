...
type: decision
status: verified
links: [ADR-003, ADR-038, C-003]
title: Official Bot API Language Set Is Closed
author: agent
accepted-by: Flemming N. Larsen (2026-08-16, Claude Code conversation)
---

# ADR-045: Official Bot API Language Set Is Closed

## Context

ADR-003 names the languages the project implements Bot APIs for, and C-003 requires every official Bot API to be 1:1 semantically equivalent to the Java reference. Neither states which languages qualify to become official, nor what happens to a Bot API written for a language outside the set.
...
- Every acceptance criterion in `bot-api/tests/TEST-REGISTRY.md` must be green on the platform in both directions, per ADR-038.
...
- [ADR-038 — Shared Cross-Platform Test Definitions](ADR-038-shared-cross-platform-test-definitions.md)
