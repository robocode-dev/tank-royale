...
The Battle Runner API ([ADR-0024](./ADR-0024-battle-runner-api.md)) waits for bot processes to connect before starting a battle. The current implementation in `BattleRunner.waitForBots()` uses **count-based** detection:
...
- [ADR-0024: Battle Runner API](./ADR-0024-battle-runner-api.md) — The Battle Runner architecture this decision extends
- [ADR-0016: Session ID for Bot Process Identification](./ADR-0016-session-id-bot-process-identification.md) — Related bot identification mechanism (session-level, not directory-level)
