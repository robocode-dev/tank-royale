...
# ADR-028: TypeScript Bot API Threading Model
...
**Problem:** JavaScript is single-threaded. There is no `Thread`, `monitor.wait()`, or `notifyAll()`. How should the TypeScript Bot API (ADR-027) replicate Java's blocking movement semantics?

**Constraints:**

- Must maintain 1:1 semantic equivalence with Java (ADR-004)
- ADR-003 states: "Different blocking behavior (`forward()` must block on ALL platforms) is unacceptable"
- The developer-facing API must be familiar across languages — no `async`/`await` leaking into bot code
- Must work in Node.js and browsers (ADR-029)
...
- The runtime abstraction layer (ADR-029) handles the difference
