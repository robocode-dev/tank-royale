...
- [IDR-005 — TypeScript captures console output, not the standard-stream pipes](IDR-005-typescript-console-capture-boundary.md) · `inferred` — TypeScript overrides `console.log`/`console.error` rather than a stream pipe, since bot code may run inside a Worker and a browser has no `process.stdout`.
