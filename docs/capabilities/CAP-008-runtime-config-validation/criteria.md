...
ame`, `version`, and `authors`.
  ...
  Scenario: Missing required properties at connection time
    ...
    And any of the required properties (
ame`, `version`, `authors`) are missing or empty
...
ame`, `version`, `authors`) are provided via code, environment variables, or a `.json` file
