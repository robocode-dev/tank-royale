...
Feature: typescript-bot-api-npm-publish — TypeScript Bot API npm publishing
  ...
  # - 
pmPack` — runs 
pm pack` to produce a local `.tgz` tarball for verification and local use
  # - 
pmPublish` — publishes to the npm registry; requires the 
pmjs-api-key` Gradle property

  # TNP-001 retired (CH-002): the 
pmPublishDryRun` task it verified was never built and
  # is not needed — 
pmPack` already produces the `.tgz` and shows what would be published
  ...
  # TNP-002 retired (CH-002): the shipped task uses the 
pmjs-api-key` Gradle property, not
  ...
  # 
pmjs-api-key` Gradle property. Reminted as TNP-006.
  @TNP-003 @retired
  ...
  Scenario: npmrc is not committed to source control
    When the 
pmPublish` task writes a temporary `.npmrc` file
  ...
  Scenario: npmPublish requires the npmjs-api-key Gradle property
    When `./gradlew :bot-api:typescript:npmPublish` is run without 
pmjs-api-key` set
  ...
  Scenario: npmPublish succeeds with a valid npmjs-api-key
    When `./gradlew :bot-api:typescript:npmPublish` is run with a valid 
pmjs-api-key` Gradle property
