...
Implemented: the 
pmPack` and 
pmPublish` Gradle tasks live in `bot-api/typescript/build.gradle.kts`, the `/release` skill drives 
pmPublish`, and the package is live on npm. Criteria stay `draft` until their tests are wired (P-001/M-002).
