...
- pmPack` task — runs pm pack` to produce a local `.tgz` tarball for verification and local use. This also covers pre-publish inspection: it shows exactly what would be published without uploading, so a separate pm publish --dry-run` task was deemed redundant and not built.
- pmPublish` task — publishes to npm with `--access public`; requires the pmjs-api-key` Gradle property (looked up from the user or project `gradle.properties`), consistent with the Sonatype/PyPI credential pattern and the `/release` skill. It writes a temporary `.npmrc` with the token and deletes it afterwards.
...
pmPublish` reads the npm token from the pmjs-api-key` Gradle property. Keep it out of the repo by putting it in your **user** Gradle properties file, never the project one:
...
The token must have publish rights to the `@robocode.dev` scope. If it is missing, pmPublish` fails with pmjs-api-key is not set in gradle.properties` (criterion TNP-005). This mirrors the other publish credentials, which live in the same file: `ossrhUsername` / `ossrhPassword` (Maven Central), uget-api-key` (NuGet), `pypiToken` (PyPI) — see the `/release` skill for the full set.
