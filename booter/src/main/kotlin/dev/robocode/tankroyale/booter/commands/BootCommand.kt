...
class BootCommand : Command() {
    ...
    private fun handleCommand(command: String, arg: String) {
        when (command) {
            "boot" -> createBotProcess(Path(arg))
            "stop" -> stopBotProcess(arg.toLong())
        }
    }
    ...
    private fun boot(bootDir: Path): Set<Process> {
        getBootEntry(bootDir)?.let { bootEntry ->
            ...
            if (bootEntry.teamMembers?.isNotEmpty() == true) {
                return bootTeam(bootDir, Team(teamId, bootEntry.name, bootEntry.version, bootEntry.teamMembers))
            }
    ...
    private fun bootTeam(bootDir: Path, team: Team): Set<Process> {
        ...
        team.members.forEach { botName ->
            ...
            findBootScriptOrNull(botDir)?.let { scriptPath ->
                bootBot(botDir, team)?.let { process ->
